...
function generateFallbackResponse(
  ...
  if (
    ...
    message.includes("điện thoại thông minh")
  ) {
  ...
  if (
    ...
    if (deals.length > 0) {
      return `Hiện tại chúng tôi có **${deals.length}** sản phẩm đang khuyến mãi! Bao gồm ${deals
        .slice(0, 2)
        .map((d) => d.name)
        .join(", ")}. Bạn quan tâm đến loại sản phẩm nào?`;
    }
