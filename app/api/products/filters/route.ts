...
export async function GET(request: NextRequest) {
  ...
  let query = supabase.from("products").select("brand, specs, category_id");
  ...
  products.forEach((p) => {
    ...
    if (specs) {
      ...
      if (specs.storage)
        storageMap.set(specs.storage, (storageMap.get(specs.storage) || 0) + 1);
      if (specs.cpu) cpuMap.set(specs.cpu, (cpuMap.get(specs.cpu) || 0) + 1);
      if (specs.screenSize)
        screenMap.set(
          specs.screenSize,
          (screenMap.get(specs.screenSize) || 0) + 1,
        ); // assuming screenSize key
    }
  });

  const formatOptions = (
    map: Map<string, number>,
    labelFn: (v: string) => string = (v) => v.toUpperCase(),
  ...
  const categoriesWithCounts = Array.from(categoryMap.entries())
    ...
    .filter(
      (item): item is { value: string; label: string; count: number } =>
        item !== null,
    )
    .sort((a, b) => b.count - a.count);
