      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <Dialog open={productDialogOpen} onOpenChange={setProductDialogOpen}>
          <DialogTrigger asChild>
            <Button onClick={() => {
              setSelectedProduct(null)
              setProductForm({ name: "", description: "", price: "", original_price: "", discount_price: "", stock: "", brand: "", category_id: "", image_url: "", images: "", is_featured: false, is_deal: false, specs: "" })
            }}>
          ...
          <DialogContent className="sm:max-w-4xl max-h-[90vh] overflow-hidden flex flex-col p-0">
            ...
            <form onSubmit={handleProductSubmit} className="overflow-y-auto px-6 py-4 space-y-8 flex-1">
              ...
              <div className="space-y-4">
                <h3 className="flex items-center gap-2 text-sm font-semibold text-primary uppercase tracking-wider"><Info className="h-4 w-4" /> Thông tin cơ bản</h3>
              ...
              {/* Section 2: Tài chính */}
              <div className="space-y-4">
                <h3 className="flex items-center gap-2 text-sm font-semibold text-primary uppercase tracking-wider"><DollarSign className="h-4 w-4" /> Giá bán & Kho</h3>
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                  ...
                  <div className="space-y-2">
                    <Label>Giá gốc</Label>
                    <Input type="number" value={productForm.original_price} onChange={e => setProductForm({ ...productForm, original_price: e.target.value })} placeholder="Tùy chọn" />
                  </div>
                  <div className="space-y-2">
                    <Label>Giá KM</Label>
                    <Input type="number" value={productForm.discount_price} onChange={e => setProductForm({ ...productForm, discount_price: e.target.value })} placeholder="Tùy chọn" />
                  </div>
                  <div className="space-y-2">
              ...
              {/* Section 3: Hình ảnh */}
              <div className="space-y-4">
                <h3 className="flex items-center gap-2 text-sm font-semibold text-primary uppercase tracking-wider"><ImageIcon className="h-4 w-4" /> Hình ảnh</h3>
                <div className="space-y-4">
                  <div className="space-y-2">
                    ...
                    <div className="flex gap-3">
                      ...
                      {productForm.image_url && <img src={productForm.image_url} alt="preview" className="h-10 w-10 rounded border object-cover" />}
                    </div>
                  </div>
                  <div className="space-y-2">
                    <Label>URL Ảnh phụ (Cách nhau bằng dấu phẩy)</Label>
                    <Textarea value={productForm.images} onChange={e => setProductForm({ ...productForm, images: e.target.value })} placeholder="https://img1.jpg, https://img2.jpg" rows={2} />
                  </div>
                </div>
              </div>
              ...
              {/* Section 4: Nâng cao */}
              <div className="space-y-4">
                <h3 className="flex items-center gap-2 text-sm font-semibold text-primary uppercase tracking-wider"><Settings2 className="h-4 w-4" /> Cấu hình nâng cao</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    <Label>Thông số kỹ thuật (JSON Format)</Label>
                    <Textarea className="font-mono text-sm" value={productForm.specs} onChange={e => setProductForm({ ...productForm, specs: e.target.value })} placeholder='{"RAM": "8GB", "CPU": "Intel i5"}' rows={4} />
                  </div>
                  <div className="space-y-4 bg-muted/30 p-4 rounded-xl border">
                    <div className="flex items-center justify-between">
                      <div className="space-y-0.5">
                        <Label>Sản phẩm nổi bật</Label>
                        <p className="text-xs text-muted-foreground">Hiển thị ở banner trang chủ</p>
                      </div>
                      <Checkbox checked={productForm.is_featured} onCheckedChange={c => setProductForm({ ...productForm, is_featured: !!c })} />
                    </div>
                    <Separator />
                    <div className="flex items-center justify-between">
                      <div className="space-y-0.5">
                        <Label>Hot Deal</Label>
                        <p className="text-xs text-muted-foreground">Gắn nhãn khuyến mãi đặc biệt</p>
                      </div>
                      <Checkbox checked={productForm.is_deal} onCheckedChange={c => setProductForm({ ...productForm, is_deal: !!c })} />
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="px-6 py-4 border-t bg-muted/20 flex justify-end gap-3">
              <Button type="button" variant="ghost" onClick={() => setProductDialogOpen(false)}>Hủy bỏ</Button>
              <Button onClick={handleProductSubmit}>{selectedProduct ? "Lưu thay đổi" : "Tạo sản phẩm"}</Button>
            </div>
          </DialogContent>
        </Dialog>
      </div>
      ...
      <div className="border rounded-xl bg-card shadow-sm overflow-hidden">
        <div className="overflow-x-auto">
          <Table>
            ...
            <TableBody>
              {productsLoading ? (
                <TableRow><TableCell colSpan={6} className="h-32 text-center text-muted-foreground">Đang tải dữ liệu...</TableCell></TableRow>
              ) : products.map((product) => (
                <TableRow key={product.id} className="hover:bg-muted/30">
                  <TableCell className="text-center">
                    {product.image_url ? (
                      <img src={product.image_url} alt={product.name} className="w-12 h-12 rounded object-cover border mx-auto" />
                    ) : (
                      <div className="w-12 h-12 bg-muted rounded flex items-center justify-center mx-auto border border-dashed"><ImageIcon className="h-5 w-5 text-muted-foreground opacity-50" /></div>
                    )}
                  </TableCell>
                  <TableCell>
                  ...
                  <TableCell>
                    ...
                    {product.original_price && <div className="text-xs line-through text-muted-foreground">{formatCurrency(product.original_price)}</div>}
                  </TableCell>
                  <TableCell className="text-center">
