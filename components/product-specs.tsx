...
export function ProductSpecs({ specifications }: ProductSpecsProps) {
  if (!specifications || typeof specifications !== 'object') {
    return (
      <div className="text-center py-12 rounded-3xl border border-dashed border-border/60 bg-muted/20">
        <p className="text-muted-foreground font-medium">Thông tin kỹ thuật chưa được cập nhật</p>
      </div>
    )
  }
  ...
  Object.entries(specifications).forEach(([key, value]) => {
    ...
    if (lowerKey.includes("cpu") || lowerKey.includes("chip") || lowerKey.includes("gpu")) category = "Hiệu năng"
    else if (lowerKey.includes("ram") || lowerKey.includes("storage") || lowerKey.includes("ssd") || lowerKey.includes("rom")) category = "Bộ nhớ"
    else if (lowerKey.includes("display") || lowerKey.includes("screen") || lowerKey.includes("màn hình") || lowerKey.includes("tần số")) category = "Màn hình"
    else if (lowerKey.includes("battery") || lowerKey.includes("pin") || lowerKey.includes("sạc")) category = "Pin & Sạc"
    else if (lowerKey.includes("camera") || lowerKey.includes("ống kính")) category = "Camera"
    else if (lowerKey.includes("os") || lowerKey.includes("android") || lowerKey.includes("ios") || lowerKey.includes("win")) category = "Hệ điều hành"
    else if (lowerKey.includes("kích thước") || lowerKey.includes("trọng lượng") || lowerKey.includes("vỏ")) category = "Thiết kế"
  ...
  return (
    <div className="grid gap-6 md:grid-cols-2">
      {Object.entries(specCategories).map(([category, categorySpecs]) => {
        ...
        return (
          <Card key={category} className="overflow-hidden border-border/40 bg-card/50 backdrop-blur-sm shadow-sm hover:shadow-md transition-all duration-300 rounded-2xl">
            {/* Header Category */}
