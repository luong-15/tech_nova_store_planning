...
const VARIANT_STYLES: Record<Variant, { bg: string; icon: React.ElementType; iconClass: string }> = {
  success: { bg: 'bg-emerald-500/15', icon: CheckCircle2, iconClass: 'text-emerald-600 dark:text-emerald-400' },
  error: { bg: 'bg-red-500/15', icon: XCircle, iconClass: 'text-red-600 dark:text-red-400' },
  warning: { bg: 'bg-amber-500/15', icon: AlertTriangle, iconClass: 'text-amber-600 dark:text-amber-400' },
  info: { bg: 'bg-blue-500/15', icon: Info, iconClass: 'text-blue-600 dark:text-blue-400' },
  loading: { bg: 'bg-slate-500/15', icon: Loader2, iconClass: 'text-slate-600 dark:text-slate-400 animate-spin' }
}
...
export function notify(
  ...
  const toastDescription = (
    <div className="flex w-full items-center gap-3.5 pr-2" role="alert" aria-live="polite">
...
export const notifyOrderPlaced = (orderNumber: string) => notifySuccess(`Đơn hàng #${orderNumber} ${MESSAGES.orderPlaced.toLowerCase()}`, { duration: 3000 })
export const notifyPaymentSuccess = () => notifySuccess(MESSAGES.paymentSuccess, { duration: 3000 })
export const notifyOutOfStock = (name: string) => notifyError(`${name}: ${MESSAGES.outOfStock.toLowerCase()}`, { duration: 3000 })
export const notifyStockLow = (name: string) => notifyWarning(`${name}: ${MESSAGES.stockLow.toLowerCase()}`, { duration: 3500 })